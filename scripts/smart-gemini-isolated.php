...
function callGemini(string $model,string $key,string $payload): array {
    ...
    // One bounded retry only for transient server failures. Never immediately
    // retry 429 on the same model; a configured backup model is tried instead.
...
try {
    ...
    foreach($models as $index=>$model){
        ...
        // A backup model is useful only for model-specific quota/capacity.
        // Auth, malformed payload and network errors should not be multiplied.
