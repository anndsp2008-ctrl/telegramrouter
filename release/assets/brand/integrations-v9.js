(() => {
  ...
  const KEY = 'telegramrouter.integrations.openProvider.v9';
  ...
  const toggle = card => {
    ...
    if (next) cards.forEach(other => {
      if (other !== card) setOpen(other, false, false);
    });
    setOpen(card, next, true);
  };
  ...
  // Provider forms submit normally: one server response, no live.js DOM replacement.
